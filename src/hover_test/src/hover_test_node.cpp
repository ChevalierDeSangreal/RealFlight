  ...
  if (current_state_ == FsmState::TRAJ && hover_started_ && !hover_completed_) {
    ...
    if (!neural_control_ready_ && elapsed >= mode_stabilization_delay_) {
      if (use_neural_control_ && policy_ && local_position_ready_ && attitude_ready_) {
        ...
        // Hovering thrust: 0.251 in [0,1] range -> -0.498 in [-1,1] range
        // Mapping: [-1,1] = 2 * [0,1] - 1
        std::vector<float> hovering_action = {-0.498f, 0.0f, 0.0f, 0.0f};
        ...
        RCLCPP_INFO(this->get_logger(), "[NORM OBS] v_body=[%.6f, %.6f, %.6f], g_body=[%.6f, %.6f, %.6f], target_distance_body=[%.6f, %.6f, %.6f]",
                    ...
                    initial_obs[6], initial_obs[7], initial_obs[8]);   // target_distance_body
...
// Based on HoverEnv observation space - all in body frame
...
std::vector<float> HoverTestNode::get_observation()
{
  ...
  // 3. 机体系到目标点距离 (target_distance_body = R^T * (target - current))
  // 注意：这里的含义是从当前位置指向目标点的向量，即"到目标点的距离"
  float target_distance_world[3] = {
    static_cast<float>(target_x_ - current_x_),
    static_cast<float>(target_y_ - current_y_),
    static_cast<float>(target_z_ - current_z_)
  };
  obs[6] = R[0][0] * target_distance_world[0] + R[1][0] * target_distance_world[1] + R[2][0] * target_distance_world[2];
  obs[7] = R[0][1] * target_distance_world[0] + R[1][1] * target_distance_world[1] + R[2][1] * target_distance_world[2];
  obs[8] = R[0][2] * target_distance_world[0] + R[1][2] * target_distance_world[1] + R[2][2] * target_distance_world[2];
